...
**Frontmatter**: `status`, `owner`, `service` or `feature`, `last_verified`, `sources` (literal files and directories), `sources_digest`, `content_digest`. `loam validate` reads it in exactly three files — a service's living `spec.md`, its living `arch.spec.md` when present (same conventions, same checks, only the label differs), and a feature's `intent.md`: the documents whose identity and status everything else joins on. `adrs/` and `runbook.md` are presence-tracked only (`loam list` says whether they exist); their frontmatter is read by nothing today, so a field written there is a note to a future reader, not a claim any check will catch. `health.yaml`'s frontmatter is equally unread, but the file itself is not inert: `loam validate` reads its top-level `slis:`/`alerts:` ids for `Covers:` resolution and the `health.uncovered` warning (see the architecture spec axis below). For the two checked files:
...
The record is YAML, like the other data artifacts, and holds the claim text next to each verdict so it reads without loam. Because it has to survive without loam, human edits are expected — and read back defensively: a file loam cannot read as a record (broken YAML, or a shape missing what the readers use — `summary` with its counts, `recorded`, `checklist`, claims each with an `id` and a known `verdict`) counts as **no record at all**, reported as "not verified" / "no verification record" rather than crashing a command or half-reading; extra keys a human adds pass through untouched. Every verdict names who answered it — `answered_by: runner` (a report's green run, mechanical) or `answered_by: agent` (somebody's word about the code) — so a reviewer can tell the two apart at a glance:
...
Three read-only fleet/transition views make that filesystem contract inspectable without creating or repairing anything:

- **`loam doctor [--json]`** reads `loam.json` defensively (including corrupt JSON), resolves its `docsDir` against the config location, probes existence/read/write access, checks `services/` and the landscape, inventories service/active-feature counts, and reports the current `service` binding. `healthy: false` and exit 1 are reserved for blockers: missing/invalid config, missing docsDir, or missing `services/`. A missing landscape, absent/unknown service binding, and access limitations are warnings, not attempted repairs. The JSON result is a successful v1.0 envelope even when unhealthy because the diagnosis itself ran; branch on `healthy` and `findings[].severity`.
...
- **`loam migrate-openspec <root> [--json]`** accepts an OpenSpec root or a workspace containing `openspec/` and performs inventory only: living capability/spec files, active and archived changes, requirement/scenario counts, `RENAMED` headings and unsupported shapes. `mechanicallyCompatible` means the Markdown/delta shapes can be carried; `ready` additionally requires every `needsMapping` capability→service ownership decision to be resolved. Since this command accepts no mapping and writes no loam files, a normal compatible OpenSpec repo reports `mechanicallyCompatible: true`, `ready: false`, and the decisions a human must make.
