...
A requirement's **name is its identity** across this whole algebra, which makes two shape rules worth stating. Two `### Requirement:` blocks with one name in one LIVING document are an **error** (`spec.duplicate-requirement`, per file — `spec.md` and `arch.spec.md` are separate namespaces, so one name in both is legal): the merge matches by name and edits only the first, so the second survives every later archive as a stale copy. And a repeated `Operations:` or `Covers:` line inside one requirement body is a **warning** (`spec.repeated-operations` / `spec.repeated-covers`, living specs and feature deltas alike): the last line REPLACES the earlier ones — assignment, not append, the documented keep-last quirk, whose semantics deliberately do not change — so the author's "long list in two lines" pattern silently loses its first line; merge them into one comma-separated line.
