...
- Cross-service use cases: flows as fleet-level dynamic views — `7943ae5`…`ab981de`.
...
Honest leftovers. The flows item closed every required change and five of its six exit criteria in
this tree; the sixth is half a fleet measurement — `validate --all` provably never enters LikeC4's
computed stage, but meeting a predeclared timing WITH journeys present is the pilot's evidence to
collect, not this tree's. Two more live outside this repository: the CI `stability` job has still to be observed
