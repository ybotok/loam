...
| Split `commands/validate.ts`'s rule functions into core | Nothing yet | ~1300 lines relocated. `core/status/` already needs these answers and does *not* re-derive them — it calls into core. Everything with two callers is already there | **Not worth it** until `loam status --service` exists |
