...
**All five models are EXTENDING models, and that is the shape this example teaches.** None of them
declares a `specification` block, none re-declares a partner, and none has a `likec4.config.json`
beside it. Each one says what is inside its service with `extend marketplace.<svc> { … }` on the
element `architecture/landscape.likec4` already binds to its directory, so `kafka`, `stripe`,
`notification-provider` and every peer service are declared exactly once — in the map — and no two
documents can disagree about their tags. The root `likec4.config.json` therefore excludes only
`**/node_modules/**` and `features/**`: `npx likec4 validate examples/docs` names ONE project,
`fleet`, holding the map and all five models, and needs no `--project` flag. The other shape — a
model that declares its own `specification` and is parsed alone, which is what every model here was
before — is still legal and still supported; SCHEMA.md's "Two shapes of a service model" has both,
the eight grades in its table — seven that came with the pair, plus the one it narrowed
(`service.likec4-config-missing`, now standalone-only) — and the
ordered recipe for migrating between them.

Two consequences are visible in the tree and worth reading for. Element ids carry their full path,
so `services/order-service/arch.spec.md` writes `Covers: marketplace.orderService.db` — the
container has no shorter name, because the model extends an element nested in the `marketplace`
group. And a cross-boundary call is written **twice** on purpose: container-level in the model
(`marketplace.notificationService.sender -> notificationProvider`) and service-level in the map
(`marketplace.notificationService -> notificationProvider`), because every fleet check — the spine,
`landscape.service-isolated`, the platform census — reads the map alone. ROADMAP.md keeps that
remainder written down rather than hidden.
