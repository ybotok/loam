...
which stamps the status, the date and both digests together — into the living spec.md
and, when the service has one, arch.spec.md, in one all-or-nothing run: a file whose
sources cannot be verified refuses the whole vouch. Never write those four fields by
hand: a status with no digest behind it is a claim with nothing behind it.
