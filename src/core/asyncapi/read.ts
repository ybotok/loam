...
import type { AsyncapiDoc, EventMessage } from "./model.js";
...
/*
 * The `EventMessage`/`AsyncapiDoc` read model lives in `./model.ts` — split
 * there when the `asyncapiFromText` extraction pushed this module past the
 * 300-line ceiling. This module keeps the walk; that one keeps what the walk
 * answers.
 */
...
export async function readAsyncapi(asyncapiPath: string, context?: FleetContext): Promise<AsyncapiDoc> {
  ...
  return asyncapiFromText(bytes.toString("utf8"));
}

/**
 * The same read over text already in hand — the parse half of `readAsyncapi`,
 * split out so `loam diff` can read a contract fetched from a base git ref
 * (core/diff/base-state.ts) without writing it to disk. The YAML-throw catch
 * lives here and nowhere else; `asyncapiDocOf` below owns everything after
 * the parse, and the byte concerns (existence, UTF-8) stay with the reader
 * that holds the bytes.
 */
export function asyncapiFromText(text: string): AsyncapiDoc {
  let doc: unknown;
  try {
    doc = parse(text);
  } catch (e) {
