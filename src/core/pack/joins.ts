/**
 * The two fleet-vocabulary joins in one service's context pack: the
 * permissions its requirements gate on, and the capabilities they realize.
 * Both resolve a per-service fact against a fleet document, which is what
 * separates them from `./living.ts` — that module never has to look past the
 * service's own directory and the fleet map.
 */
