...
| \`delta.service-id-invalid\` | a \`specs/<svc>/\` DIRECTORY NAME that is not a legal service id (a space, a \`..\`, a Windows-reserved stem like \`NUL\`, a trailing dot) — archive would materialise \`services/<svc>/\` from it, a directory every loam command then refuses to address (\`service.id-invalid\`). Never suspended: the name is illegal whatever the delta says | rename the specs/ directory to a legal id; \`--approve\` does not override this one — the breakage is mechanical, not a judgment call |
