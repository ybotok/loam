...
| \`verify.checklist-forked\` (warn) | two or more services on this record answered DIFFERENT versions of the feature's question set — each attestation now carries the \`checklist\` digest it answered, and they disagree. Normal mid-rollout: services attest as they finish, and a feature legitimately changes between the first and the last. Attestations written before the field existed carry no digest and are not counted as a third version | re-record the services on the older version from their own repositories (\`loam verify <FEAT> --service <id> --record\`), or accept the split knowingly — nothing gates on it |
