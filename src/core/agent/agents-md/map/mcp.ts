 ...
 */
export const MCP_COMMAND = `- \`loam mcp\` serves the fourteen base read commands — validate, status, list, show,
  ...
  the command's stdout. The orientation and instruction pages are MCP resources,
  and every tool declares the envelope output schema. Every base tool is advertised with
  ...
  fetch the protocol its own skill points at. An explicit \`mcp --author\` adds
  \`new\`, \`rebase\`, \`gherkin\` and an archive dry-run. It never exposes
  \`vouch\`, evidence recording or a committing archive; those stay in the CLI,
  where the permission flow is yours.
`;
