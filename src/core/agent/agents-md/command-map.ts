...
- \`loam validate --all\` runs both of those for everything and adds the fleet
  ...
  missing or unreadable), \`service.likec4-config-missing\` (warn, per
  service: a model with no \`likec4.config.json\` beside it renders nowhere from
  the docs root — \`loam subsystem sync\` writes the file), a per-service \`sources.unverifiable-from-here\`, and
