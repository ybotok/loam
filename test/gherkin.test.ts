...
describe("feature mode", () => {
  ...
  it("the stamp line is a LITERAL on-disk contract, pinned byte for byte", () => {
    // Every other stamp assertion in this suite builds its expectation by
    // calling gherkinStampLine — so a reworded stamp plus its reworded parse
    // regex would sail through the whole suite while orphaning every .feature
    // already emitted into service repos loam does not control: old files
    // would stop parsing as loam's, feature-scope cleanup would abandon them,
    // and whole existing suites would grade gherkin.missing after an upgrade.
    // If this assertion fails you are changing a CROSS-VERSION on-disk format:
    // teach the parser to accept BOTH spellings first, then bump this literal
    // deliberately.
    expect(gherkinStampLine("0.0.0")).toBe(
      "# generated by loam v0.0.0 — `loam gherkin`; do not hand-edit, regenerate instead",
    );
    // and the parser recognizes exactly those bytes back (the round trip old
    // files depend on)
    const parsed = parseStampedFeature(
      "# generated by loam v0.0.0 — `loam gherkin`; do not hand-edit, regenerate instead\nFeature: X\n",
    );
