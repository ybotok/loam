...
| Agent surface | skills + commands for 34 tools; `openspec update` regenerates them in place | skills + commands for 20 tools; `--json` on every command; generated files are never regenerated, only reported (`doctor.agent-files-missing`) |
...
| `loam status [<FEAT>]` | Where the work stands and what to do next — the question an agent has when it joins a repo halfway or loses its session. Artifacts come back `missing`/`blocked`/`draft`/`ready`/`done`, and `next[]` is ordered, each entry a stable code plus the literal command. Derived every run from the files; there is no state file to go stale. Writes nothing. |
