...
| `loam flow <verb> [group]` | Cross-service journeys — LikeC4 dynamic views authored one per file under `architecture/flows/`, grouped into suites by TAG (many-to-many; an undeclared tag is a parse error, so a mistyped suite cannot silently open a second one). `sync` regenerates `architecture/flow-groups.likec4`, one view per group whose members are the union of that group's flows' participants, and removes it when no flow declares a group. `env [group]` answers the same union as the environment a run needs: which services must be up, with every participant that models no service directory named rather than dropped. |
