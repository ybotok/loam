<!-- generated by loam v0.2.0-alpha.6 -->
...
hand-edited and never a second fleet map; its one styling opt-in is a
`global { styleGroup subsystems { … } }` declared in any `.likec4` under
`architecture/`, which `sync` then references. A diagram you author belongs in the
landscape or in a file of your own name beside it.
`services/<id>/` holds a boundary's living `model.likec4`, `spec.md`,
`arch.spec.md`, OpenAPI, AsyncAPI, runbook and health facts — and
`services/<…>/<id>/usecases/*.likec4`, the slot for a `dynamic view` over that
service's OWN containers, tagged `#req-<Requirement-ID>`; a flow that crosses
services lives in `architecture/usecases/` and is tagged `#cap-` instead.
...
writer or uploading anything. Reports are numbered — `loam-reports/NNN-YYYY-MM-DD-<slug>.md`,
cited by ordinal — and carry a `Status:` line, so `loam doctor` counts the
directory and says which ordinal the next report takes. This is support, not a
seventh lifecycle step.
...
1. **Adopt** — for an undocumented boundary load `loam instructions loam-adopt
   ...
   never an agent, later vouches for them. Draw the map's edges in the same pass:
   the brief's `landscape.touched` is the only place an edgeless element is named,
   and `landscape.service-isolated` (fleet scope) goes quiet on the first edge.
